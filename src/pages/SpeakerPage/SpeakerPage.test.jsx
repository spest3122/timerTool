...
describe("(3) listen-pause feature – playback with SpeechRecognition", () => {
    it("shows 🎙️ Listening status when a (3) line is encountered during playback", async () => {
        ...
        global.speechSynthesis.speak = vi.fn((utt) => {
            utt.onend?.();
        });

        fireEvent.click(screen.getByRole("button", { name: /Play Text/i }));

        await waitFor(() => expect(screen.getByText(/Listening 3 s/i)).toBeInTheDocument());
    });
...
describe("SpeakerPage – permanent remark highlight", () => {
    it("highlights selected text as a remark and toggles it off when clicked again", async () => {
        renderSpeakerPage();
        const textarea = screen.getByPlaceholderText(/Type something here/i);
        fireEvent.change(textarea, { target: { value: "Hello world from speaker" } });

        // Simulate selection of 'world' (indices 6 to 11)
        textarea.selectionStart = 6;
        textarea.selectionEnd = 11;

        const highlightBtn = screen.getByTitle(/Highlight selected text as a remark/i);
        fireEvent.click(highlightBtn);

        // Backdrop should contain a <mark> with the content 'world'
        const backdrop = document.querySelector(".spk-textarea-backdrop");
        expect(backdrop).toBeInTheDocument();
        const mark = backdrop.querySelector("mark");
        expect(mark).toBeInTheDocument();
        expect(mark.textContent).toBe("world");

        // Click again with the same selection to toggle it off
        fireEvent.click(highlightBtn);
        expect(backdrop.querySelector("mark")).toBeNull();
    });

    it("correctly shifts remark indices when text is typed/inserted before the highlight", async () => {
        renderSpeakerPage();
        const textarea = screen.getByPlaceholderText(/Type something here/i);
        fireEvent.change(textarea, { target: { value: "Hello world" } });
        ...
        const backdrop = document.querySelector(".spk-textarea-backdrop");
        expect(backdrop.querySelector("mark").textContent).toBe("world");

        // Type a word 'beautiful ' at index 6 (length 10)
        fireEvent.change(textarea, { target: { value: "Hello beautiful world" } });

        // The highlight should shift to index 16 to 21, and its content should still be 'world'
        await waitFor(() => {
            const mark = backdrop.querySelector("mark");
            expect(mark).toBeInTheDocument();
            expect(mark.textContent).toBe("world");
    ...
    it("clears all highlights when the clear button is clicked", async () => {
        renderSpeakerPage();
        const textarea = screen.getByPlaceholderText(/Type something here/i);
        fireEvent.change(textarea, { target: { value: "Hello world" } });
        ...
        const backdrop = document.querySelector(".spk-textarea-backdrop");
        expect(backdrop.querySelector("mark")).toBeInTheDocument();
